<template>
  <div class="sidebar shadow-xl ">
      <div class=" py-4 bar-items flex flex-col">
        <a href="/" class=" py-4 flex bar-item align-middle justify-center items-center " >
            <svg class=" fill-current text-gray-600" xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M19 9.3V4h-3v2.6L12 3 2 12h3v8h5v-6h4v6h5v-8h3l-3-2.7zm-9 .7c0-1.1.9-2 2-2s2 .9 2 2h-4z"/></svg>
            <h3 class="bar-text hidden ml-6 text-gray-600 font-bold">Home</h3>
        </a>
        <div class=" search-bar py-4 flex bar-item align-middle justify-center items-center " >
            <svg class=" fill-current text-gray-600" xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zm6.93 6h-2.95a15.65 15.65 0 00-1.38-3.56A8.03 8.03 0 0118.92 8zM12 4.04c.83 1.2 1.48 2.53 1.91 3.96h-3.82c.43-1.43 1.08-2.76 1.91-3.96zM4.26 14C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.32-.14 2s.06 1.34.14 2H4.26zm.82 2h2.95c.32 1.25.78 2.45 1.38 3.56A7.987 7.987 0 015.08 16zm2.95-8H5.08a7.987 7.987 0 014.33-3.56A15.65 15.65 0 008.03 8zM12 19.96c-.83-1.2-1.48-2.53-1.91-3.96h3.82c-.43 1.43-1.08 2.76-1.91 3.96zM14.34 14H9.66c-.09-.66-.16-1.32-.16-2s.07-1.35.16-2h4.68c.09.65.16 1.32.16 2s-.07 1.34-.16 2zm.25 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95a8.03 8.03 0 01-4.33 3.56zM16.36 14c.08-.66.14-1.32.14-2s-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2h-3.38z"/></svg>
            <h3 class="bar-text hidden ml-6 text-gray-600 font-bold">Country</h3>
        </div>
        <div id="autocomplete-id">
            <autocomplete :search="search" :auto-select="true" placeholder="Search country..." @submit="gotocountry"></autocomplete>
        </div>
      </div>
  </div>
</template>

<script>
import countriesdata from '~/static/countries'
import countrynames from '~/static/convertcode'

export default {
data(){
    return{
        countries: countriesdata,
        isoCountries : countrynames
    }
},
methods: {
    search(input) {
        if (input.length < 1) {
            return []
        }
        return countriesdata.filter(country => {
            return country.toLowerCase().startsWith(input.toLowerCase())
        })
    },
    gotocountry(result){
        const country_code = this.getCountryCode(`${result}`)
        window.location = `/country/${country_code}`
    },
    getCountryCode(countryName) {
    if (this.isoCountries.hasOwnProperty(countryName)) {
        return this.isoCountries[countryName];
    } else {
        return countryName;
    }
    }
}
}
</script>


<style>
.sidebar {
  height: 100%;
  width: 50px;
  position: fixed;
  z-index: 100000;
  top: 0;
  right: 0;
  background-color: #ffffff;
  overflow-x: hidden;
  transition: 0.5s;
  /* transform: translateX(150px); */
  padding-top: 60px;
}

.sidebar:hover {
    /* transform: translateX(0); */
    width: 250px;
    transition: 0.5s;
}
.bar-item{
    /* transform: translateX(0); */
    transition: 0.5s;
}

.sidebar:hover .bar-text{
    /* transform: translateX(50px); */
    transition: 0.5s;
    display: block;
}

.search-bar:hover{
    cursor: pointer;;
}

#autocomplete-id {
  width: 200px;
  max-width: 200px;
  margin: 0 auto;
}

</style>